...
namespace GraphDB.Tool.Layout
{
    class CircleLayout
    {
        ...
        public CircleLayout(int iNum, int iRadius)
        {
            ...
            else if (myNum1StLevel <= 20)//连边长度上限
            {
                myLineLength = Convert.ToInt32(nodeDistance / (2 * Math.Sin(Math.PI / myNum1StLevel)));
            }
            else
            {
                myLineLength = Convert.ToInt32(nodeDistance / (2 * Math.Sin(Math.PI / 20)));
            }
        ...
        private Point GetCenterPoint(int index, Graph subgraph)
        {
            ...
            Node curNode = subgraph.Nodes.ElementAt(index).Value;

            foreach (Edge edge in curNode.OutBound)
            {
                Node tarNode = edge.To;

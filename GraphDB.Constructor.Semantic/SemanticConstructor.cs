...
namespace GraphDB.Constructor.Semantic
{
    public class SemanticConstructor
    {
        ...
        private void AddNewHanzi( string newChar )
        {
            ...
            mySemanticGraph.AddNode( newHanzi );
        }
        ...
        private void AddRelation(string lastHanzi, string curHanzi)
        {
            ...
            //Next
            Edge next = mySemanticGraph.GetEdgeByType( lastHanzi, curHanzi, CommonStrings.Next );
            if( next == null )
            {
                next = new Next();
                mySemanticGraph.AddEdge(lastHanzi, curHanzi, next);
            }
            else
            {
                ((Next) next).AddWeight();
            ...
            //Previous
            Edge previous = mySemanticGraph.GetEdgeByType(curHanzi, lastHanzi, CommonStrings.Previous);
            if (previous == null)
            {
                previous = new Previous();
                mySemanticGraph.AddEdge(curHanzi, lastHanzi, previous);
            }
            else
            {
